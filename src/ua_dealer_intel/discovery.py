    ...
    for location in DISCOVERY_LOCATIONS:
        ...
        for query in location["queries"]:
            ...
            for provider in DISCOVERY_PROVIDERS:
                ...
                provider_records = parse_discovery_results(
                    html=html,
...
def discover_from_official_sources(
    ...
    for source in DISCOVERY_OFFICIAL_SOURCES:
        ...
        parsed_records = parse_official_directory(
            html=html,
            page_url=url,
...
def parse_discovery_results(
    ...
    if provider_name == "duckduckgo_html":
        items = _parse_duckduckgo(soup, city, region, query, provider_name)
    elif provider_name == "bing":
        items = _parse_bing(soup, city, region, query, provider_name)
    else:
        items = []
    return items
